...
create_endpoint () {
  case $1 in
    ...
    ;;
    image)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$EXT_HOST_IP"':9292/v1' --adminurl 'http://'"$HOST_IP"':9292/v1' --internalurl 'http://'"$HOST_IP"':9292/v1'
    ;;
    identity)
